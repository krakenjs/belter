...
function isEventUnique(name: string): boolean {
  return getBelterExperimentStorage().getSessionState(
    (state: { loggedBeacons: string[] }) => {
      state.loggedBeacons = state.loggedBeacons || [];

      if (!state.loggedBeacons.includes(name)) {
...
type Payload = Record<string, (string | boolean) | undefined | undefined>;
...
function getThrottlePercentile(name: string): number {
  return getBelterExperimentStorage().getState(
    (state: { throttlePercentiles: Record<string, unknown> }) => {
