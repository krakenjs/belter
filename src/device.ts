...
export function getUserAgent(): string {
  // @ts-expect-error mockUserAgent is not real
...
export function isDevice(userAgent: string = getUserAgent()): boolean {
  if (
    /Android|webOS|iPhone|iPad|iPod|bada|Symbian|Palm|CriOS|BlackBerry|IEMobile|WindowsMobile|Opera Mini/i.exec(
      userAgent
    )
  ) {
    return true;
...
export function isStandAlone(): boolean {
  return (
    // @ts-expect-error standalone property does not exist on navigator
...
export function isSFVC(ua: string = getUserAgent()): boolean {
  if (isIos(ua)) {
    ...
    if (isIOS14(ua)) {
      // @ts-expect-error window height as indexer
...
export function isSafari(ua: string = getUserAgent()): boolean {
  return (
    ua.includes("Safari") && !isChrome(ua) && !/Silk|FxiOS|EdgiOS/.test(ua)
  );
}
...
export function isCrossSiteTrackingEnabled(expectedCookieKey: string): boolean {
  return !window.document.cookie.includes(expectedCookieKey);
}
